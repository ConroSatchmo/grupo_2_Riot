...
password.addEventListener('blur', (e) => {
    if(!password.value.match(/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/)){
        if(password.value.length == 0){
            ...
            errors['password'] = 'La contraseña debe tener al menos 8 caracteres, una letra mayuscula, una minuscula, un caracter especial y un numero'
            password.classList.add('border-danger')
            password.classList.remove('shadow-danger')
            passwordV.innerHTML = errors.password
        }
...
image.addEventListener('change', (e) => {
    const type = image.files[0].type
    if(type != 'image/png' && type != 'image/jpeg' && type != 'image/jpg' && type != 'image/gif'){
        errors['image'] = 'La imagen debe ser una imagen png, jpg, jpeg o gif'
        image.classList.add('border-danger')
        image.classList.add('shadow-danger')
